...
拾光笺自己只留音频和转写。配了 `download.video_dir` 之后多一组 `/api/downloads*` 接口，把整条视频合流成 mp4 存进那个目录（按 `<站点>/<作者>/<标题> [id].mp4` 分层），旁边写一个精简的 `<同名>.suishi.json`（原链接、作者、完整标题、下载时间）。这是给同一台机器上的 [随拾 · 本地短视频抽屉](../local-video-browser) 用的：随拾把这个目录加进抽屉，就能像刷本地视频一样刷这些存档，并且一键把它们送回拾光笺转写。

- 下载走独立队列，不排在转写后面；相邻两次仍隔 `batch_delay_sec`，B 站 412 按 IP 算
- 同一 `video_id` 只留一份，文件被手动删了才允许重下；记录在 `cache.sqlite` 的 `downloads` 表，重启接着下
- 清晰度上限 `download.video_quality`（默认 1080），默认优先 H.264 让浏览器能原生播（`prefer_h264`）
- 需要本机有 `ffmpeg`（B 站 1080p 是音视频分离的 DASH）
- 跨域来源要写进 `web.cors_origins`，只接受 localhost；接口列表见 `/api/docs`
