# 拾光笺（video-summarizer）

给一个视频链接（YouTube / Bilibili / 其他 yt-dlp 支持的站点），自动产出**结构化转写**、**大模型总结**和**思维导图**，处理过的视频按 UP 主攒成一个库。命令行叫 `vsum`。

设计细节见 [DESIGN.md](DESIGN.md)。路线图 v0.1 – v0.5 已完成，v0.6（界面重构）进行中。
...
FastAPI 后端 + React 前端，构建产物随包分发，**用户不需要装 Node**。三个页面：
...
校验比 prompt 可靠：`from` 必须在原文里原样出现、不含数字（数字错交给人）、长度差不超过 2 字、不许带句末标点、不许链式替换，不过的一律丢。否决过的条目重跑也不会再冒出来。搜索索引原文和修正文都进，记得哪个写法搜哪个都能命中；总结和问答用修正后的正文，替换表变了总结缓存自动失效。只对语音识别来源生效，官方字幕不跑；ollama 本地小模型跳过。一小时视频约 ¥0.03、十几秒。默认关：`summarizer.correct_terms: true` 打开，或每次处理时勾选。
...
见 [DESIGN.md 第 8 节](DESIGN.md)。v0.1 – v0.5 全部完成；v0.6 界面重构（FastAPI + React + 任务队列）已落地，接下来是 v0.7 的全文搜索、问视频、批量。
