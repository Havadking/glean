...
- **处理** — 贴链接 → 转写 → 总结。分两步：先出转写和预计消耗，确认后再点「生成总结」才会真正调大模型，和命令行的成本确认同一个道理。
- **历史** — 处理过的视频，以视频名为标题的卡片列表。带搜索和排序，卡片上的徽章显示已经生成过哪些总结。点「打开」切到详情（列表隐藏、页面回顶），「返回列表」回来。

转写有两种视图。**阅读**把碎分句聚成段落 —— 字幕和 ASR 出来的分句都是两三秒一条、二三十个字，一个 30 分钟的视频有 600 多行，等于让人一行行读字幕；聚合后实测 175 句变 14 段、平均 312 字，段首带时间戳锚点。**精确**是原始分句表格，用来查某句话具体在第几秒。聚合只影响显示，`transcript.json` 保持精确时间轴不变。
