...
SYSTEM = (
    ...
    "不要逐字复述。如果某处转写明显有误或听不清，可以标注为“(转写不清)”。\n\n"
    # 实测教训：模型会从标题/URL 认出视频，然后大段输出转写里根本没有的"背景知识"
    # （上传者、上传日期、播放量……），且语气笃定，非常难辨认。这条约束必须写死。
    "**只依据转写内容作答。** 即使你认得这个视频、这个人或这个话题，也不要写入任何"
    "转写里没有出现的信息 —— 包括上传者、发布时间、播放量、事件背景、人物履历、"
    "后续发展等等。这些即便你记得也可能是错的，而且读者无法分辨哪些来自视频、哪些来自你。"
    "元信息里给出的标题和链接只是让你知道在处理哪个视频，不是可以引用的内容来源。"
    "转写内容少，总结就短；宁可写“视频内容有限，没有更多可总结的”，也不要用背景知识填充篇幅。"
)
...
MINDMAP = PromptTemplate(
    key="mindmap",
    label="思维导图",
    instruction="""请把内容整理成一份思维导图的大纲。**只输出 Markdown 大纲本身**，不要任何解释、开场白或代码块。
...
规则：
- 节点要短：一级分支 2-8 个字，二级节点不超过 20 字，三级以下不超过 30 字。导图是给人一眼看结构的，不是读段落的
- 每个节点只表达一个意思，不要用"和"把两件事塞在一起
- 关键数字、结论、行动项放在叶子节点，让它们能被一眼找到
- 只用 `#`/`##` 标题和 `-` 列表，缩进用两个空格。不要用表格、引用、加粗、链接
- 不要为了凑分支编造内容。内容少就少几个分支""",
    map_instruction="""这是一段长转写中的一部分。请只用 `##` 标题和 `-` 列表（两个空格缩进）
把这一段的话题和要点整理成大纲片段，节点要短。不要写根标题，不要写开场白和结语。""",
)

TEMPLATES: dict[str, PromptTemplate] = {
    t.key: t for t in (OVERALL, BY_SPEAKER, TIMELINE, KEY_POINTS, MINDMAP)
}
